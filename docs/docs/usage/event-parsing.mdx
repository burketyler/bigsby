...
By default, Bigsby will match the argument name to the query string name. You can change this behaviour by specifying
the query string name when calling `@Query`.
...
By default, Bigsby will match the argument name to the path parameter name. You can change this behaviour by specifying
the path parameter name when calling `@Param`.
...
By default, Bigsby will match the argument name to the header name. You can change this behaviour by specifying
the header name when calling `@Header`.
...
contains things that may be useful in some scenarios such as the original event payload, the handler's
configuration and the Bigsby instance the handler belongs to.

The request context can also be extended to suit your needs, you may use a plugin or lifecycle hook that adds
metadata or information to the request for consumption during execution.
...
will still execute though, so it's up to you to add the necessary [validation](./validation) or include null
